	...
	def train( self, trainingData, trainingLabels, validationData, validationLabels, validate):
		...
		for iteration in range(self.max_iterations):
			...
			for i in range(len(trainingData)):
				...
				# "*** YOUR CODE HERE ***"
				for all_labels in range(len(self.legalLabels)):
					if trainingLabels[i] > all_labels:
						if self.weights[frozenset([trainingLabels[i], all_labels])] * trainingData[i] <= 0:
							self.weights[frozenset([trainingLabels[i], all_labels])] += trainingData[i]
					if trainingLabels[i] < all_labels:
						if self.weights[frozenset([trainingLabels[i], all_labels])] * trainingData[i] > 0:
							self.weights[frozenset([trainingLabels[i], all_labels])] -= trainingData[i]
