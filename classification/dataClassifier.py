...
def enhancedFeatureExtractorDigit(datum):
	...
	"*** YOUR CODE HERE ***"
	def sumPixelVal(start_index, end_index):
		sum_val = 0
		for i in range(start_index, end_index):
			sum_val = sum_val + datum[i]
		return sum_val
	...
	dark_point_near = findCornor()
	# print(dark_point_near)
	# plt.plot(dark_point_near)
	# plt.show()

	def calcArea():
		dark_points = 0
		for i in range(DATUM_WIDTH * DATUM_HEIGHT):
			dark_points = dark_points + datum[i]

		return dark_points

	def calcPerimeter():
		#left view calculation
		total = 0
		for i in range(DATUM_WIDTH * DATUM_HEIGHT):

			if datum[i] == 1 and (datum[i-1] == 0 or datum[i+1] == 0 or datum[i-51] == 0 or datum[i+51] == 0 or datum[i-49] == 0 or datum[i+49] == 0):
				total += 1
	...
	for i in range(1):
		#This gives acc of 60.5
		#features[i] = (sumPixelVal((i+1)*250, (i+2)*250) - sumPixelVal((i)*250, (i+1)*250))*(sumPixelVal((i+1)*250, (i+2)*250) - sumPixelVal((i)*250, (i+1)*250))
